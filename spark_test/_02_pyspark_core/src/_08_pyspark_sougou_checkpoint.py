...
if __name__ == '__main__':
    ...
    # 6.1 设置检查点在HDFS上的存储位置. 虽然它也可以存储到本地Linux系统, 但是无意义(因为本地模式我们主要是测试用的, 测试时数据量一般不会特别大, 无意义)
    ...
    # 6. 设置检查点
    # 6.2 开启检查点
    rdd_map.checkpoint()   # 因为 checkpoint() 没有任何返回结果, 所以这里不能写成: rdd_map.checkpoint().count()
    rdd_map.count()        # checkpoint() 检查点函数是Lazy, 需要遇到 动作算子(Action)才会执行.
